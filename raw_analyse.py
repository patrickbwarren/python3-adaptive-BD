...
df2 = df[['Q', v, 'block', 'Δr2']].groupby(['Q', v, 'block']).mean() # calculate mean square displacement per block
...
ser1 = df2[['Q', v, 'RMSD']].groupby(['Q', v]).mean()['RMSD'] # the global mean RMSD
ser2 = df2[['Q', v, 'RMSD']].groupby(['Q', v]).sem()['RMSD'].rename('std_err') # std error in mean

df3 = pd.concat([ser1, ser2], axis=1).reset_index() # compile these into a new dataframe, pulling index (Q) back to a column

if args.output:
    with pd.ExcelWriter(args.output) as writer:
        for x in df3[v].unique():
            df3[df3[v] == x].drop(v, axis=1).to_excel(writer, sheet_name=f'{v}={x}', index=False)
    cols = ', '.join([str(x) for x in df[v].unique()])
    print(f'Data for {v} = {cols} written to', args.output)
else:
    print(df3)
