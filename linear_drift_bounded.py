...
parser = argparse.ArgumentParser(description='Chandrasekhar sedimentation adaptive BD simulator')
...
parser.add_argument('--z0', default=50.0, type=float, help='initial position in z, default 50.0 um')
parser.add_argument('--gamma', default=0.2, type=float, help='constant drift speed in -z direction, default 0.2 um/s')
...
parser.add_argument('-t', '--tfinal', default='900', help='duration, default 900 sec')
...
# Constant drift field: γ in negative z direction (reflected).
...
adb = adaptive_bd.Simulator(rng=local_rng, drift=reflected_drift)
